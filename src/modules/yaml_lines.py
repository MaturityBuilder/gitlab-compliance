...
def index_yaml_text(text: str) -> dict:
    """Build a line index from YAML text already loaded in memory."""
    jobs: dict[str, int] = {}
    includes: list[int] = []
    variables: dict[str, int] = {}
    workflow_rules: list[int] = []

    for document in compose_all(StringIO(text), Loader=EnvLoader):
...
def index_yaml_file(config_file: str) -> dict:
    with open(config_file, encoding="utf-8") as handle:
        return index_yaml_text(handle.read())
