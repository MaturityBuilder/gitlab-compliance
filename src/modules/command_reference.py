...
def _format_default(default, required: bool) -> str:
    if required and str(default).lower() == "sentinel.unset":
        return "required"
    if default is None:
        return "not set"
...
def _format_options(options: dict) -> str:
    ...
    for opt_name, opt in options.items():
        required_text = "yes" if opt.get("required") else "no"
        description = opt.get("help") or "No description provided."
        kind_label = "Argument" if opt.get("kind") == "argument" else "Option"
        sections.append(
            "\n".join(
                [
                    f"### `{opt.get('usage')}`",
