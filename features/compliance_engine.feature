Feature: Compliance engine
  ...
  Scenario: Passing policy succeeds against sample pipeline
    When I run basic compliance on "sample-files/.gitlab-ci.yml" with policies "tests/compliance_policies/passing"
    Then compliance should pass

  Scenario: Failing policy returns non-zero exit
    When I run basic compliance on "sample-files/.gitlab-ci.yml" with policies "tests/compliance_policies/failing"
    Then compliance should fail

  Scenario: Empty WHEN filter skips scenario
    When I run basic compliance on "sample-files/.gitlab-ci.yml" with policies "tests/compliance_policies/skip"
    Then compliance should pass with skipped scenarios

  Scenario: Missing API connection skips API-backed scenario
    When I run api compliance on "sample-files/.gitlab-ci.yml" with policies "tests/compliance_policies/api-missing"
    Then compliance should pass with skipped scenarios

  Scenario: Missing API connection fails in strict mode
    When I run strict api compliance on "sample-files/.gitlab-ci.yml" with policies "tests/compliance_policies/api-missing"
    Then compliance should fail
