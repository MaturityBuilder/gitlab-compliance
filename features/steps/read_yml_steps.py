...
def step_create_yaml_file(context, filename):
    context.temp_file = tempfile.NamedTemporaryFile(mode="w+", delete=False, suffix=".yml")
...
def step_check_document_count(context, count):
    assert len(context.documents) == count, f"Expected {count} documents, got {len(context.documents)}"
