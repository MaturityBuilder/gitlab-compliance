...
def step_create_table_with_field_names(context, headers, field_names):
    ...
    context.pretty_table = table_design(headers=headers_list, field_names=field_names_list)
...
def step_check_field_names(context, expected):
    ...
    assert context.pretty_table.field_names == expected_list, \
...
def step_check_alignment(context):
    for col in context.pretty_table.field_names:
        assert context.pretty_table.align[col] == "c", f"Column '{col}' is not center-aligned"
