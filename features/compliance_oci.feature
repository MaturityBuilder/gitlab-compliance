Feature: Compliance OCI policy bundles
  ...
  Scenario: Policy bundle can be created and extracted locally
    When I bundle and extract policies from "example-policies/security" to "/tmp/gitlab-docs-policy-bundle-test"
  ...
  Scenario: OCI references are detected separately from local paths
    ...
    When I check whether "example-policies/security" is an OCI reference
