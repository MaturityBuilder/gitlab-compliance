Feature: Compliance OCI policy bundles
  ...
  Scenario: Policy bundle can be created and extracted locally
    When I bundle and extract policies from "examples/example-policies/security" to "/tmp/gitlab-compliance-policy-bundle-test"
  ...
  Scenario: OCI references are detected separately from local paths
    ...
    When I check whether "examples/example-policies/security" is an OCI reference
