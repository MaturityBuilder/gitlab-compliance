...
class TestPolicyFixRegistry:
    def test_supported_ids_cover_documented_remediations(self):
        ...
        assert "GLCI-BUILTIN-IMAGE-01" in ids
        assert "GLCI-BUILTIN-INCLUDE-03" in ids
        assert "GLCI-BUILTIN-INCLUDE-01" not in ids

    def test_kind_lookup(self):
        assert pf.remediation_kind_for_policy("GLCI-BUILTIN-IMAGE-01") == "image_digest"
        assert (
            pf.remediation_kind_for_policy("GLCI-BUILTIN-INCLUDE-04")
            == "include_latest"
        )
        ...
        messages = pf.apply_policy_remediations(
            [
                ScenarioResult(
                    ...
                    name="grace",
                    status="failed",
                    policy_id="GLCI-BUILTIN-INCLUDE-04",
                )
            ],
            pipeline_file=str(SAMPLE_PIPELINE),
            token="token",
        ...
        messages = pf.apply_policy_remediations(
            [
                ScenarioResult(
                    ...
                    name="latest",
                    status="failed",
                    policy_id="GLCI-BUILTIN-INCLUDE-03",
                )
            ],
            pipeline_file=str(SAMPLE_PIPELINE),
            token="token",
