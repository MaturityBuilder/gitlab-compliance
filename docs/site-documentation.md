...
Developer documentation for **GitLab Compliance** is built with [Zensical](https://zensical.org/). Site configuration is in [`mkdocs.yml` on GitHub](https://github.com/MaturityBuilder/gitlab-compliance/blob/main/mkdocs.yml) at the repository root (Zensical reads this format natively).
...
Zensical only validates links to files under `docs/`. Link to repository paths outside that folder with full URLs (for example `examples/example-policies/` on GitHub), not `../` relative paths.

## GitLab CI/CD

| Job | Stage | When | Purpose |
| --- | --- | --- | --- |
| `docs:review` | `build` | Merge requests and default branch | `zensical build --strict`; upload `public/` artifact for review |
...
| Job | When | Purpose |
| --- | --- | --- |
| `review` | Pull requests to `main` / `master` | `zensical build --strict`; upload `docs-preview` artifact |
