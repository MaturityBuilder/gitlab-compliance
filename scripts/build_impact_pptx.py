...
def _footer(slide, page: int) -> None:
    _text(
        ...
        "Impact briefing  ·  Dedicated migration · Nexus · tech debt",
        size=Pt(11),
        color=MUTED,
    )
    _text(
...
def build() -> Path:
    ...
    _text(
        _box(s, Inches(0.85), Inches(3.5), Inches(11), Inches(1.0)),
        "How check, local shell scans, supply-chain remediations, and documentation "
        "commands create visibility and reduce migration risk.",
        size=Pt(17),
    ...
    _lines(
        ...
        [
            ...
            ("Focus: check · shell scan · supply chain · document/generate", SEA_SOFT, False, Pt(13)),
        ],
    )
    ...
    items = [
        ...
        "4.  Deep dive on each capability and the outcome it creates",
        "5.  End-to-end debt discovery → remediate → prove compliance",
        "6.  Recommended rollout for migration programmes",
    ]
    ...
    s = prs.slides.add_slide(blank)
    _bg(s)
    _header(s, "Capability map", "Four levers — one outcome: visible, fixable debt")
    _footer(s, 5)
    caps = [
    ...
    for i, (title, how, impact) in enumerate(caps):
        ...
        _rect(s, Inches(0.65), top, Inches(2.4), Inches(1.08), SEA)
        _text(_box(s, Inches(0.75), top + Inches(0.32), Inches(2.2), Inches(0.45)), title, size=Pt(15), bold=True, color=WHITE, align=PP_ALIGN.CENTER)
        _text(_box(s, Inches(3.3), top + Inches(0.15), Inches(9), Inches(0.35)), how, size=Pt(14), color=BODY)
        _text(_box(s, Inches(3.3), top + Inches(0.55), Inches(9), Inches(0.35)), f"Impact: {impact}", size=Pt(13), bold=True, color=SEA)
    ...
    _bullets(
        ...
        [
            ...
            "Warn-only first (allow_failure), then enforce — same policies, stronger gate",
        ],
        size=Pt(15),
    )
    _card(s, Inches(0.65), Inches(4.0), Inches(12), Inches(2.4))
    _text(_box(s, Inches(0.95), Inches(4.2), Inches(11.4), Inches(0.35)), "Migration / Nexus impact", size=Pt(15), bold=True, color=INK)
    _bullets(
        ...
        [
            ...
            "Surfaces debt early enough to schedule remediation inside the migration plan",
        ],
        size=Pt(14),
    )
    ...
    steps = [
        ...
        ("Gate", "Move from allow_failure to blocking MR checks; optionally org-wide PEP later."),
    ]
    ...
    rows = [
        ...
        ("Sustainable remediation", "Scheduled --fix-supply-chain --create-mr jobs"),
    ]
    ...
    s = prs.slides.add_slide(blank)
    _bg(s)
    _header(s, "End-to-end: discover → fix → prove", "One toolchain across migration and compliance workstreams")
    _footer(s, 12)
    flow = [
        ...
        ("6. Enforce", "CI gate + optional Pipeline Execution Policy"),
    ]
    ...
    s = prs.slides.add_slide(blank)
    _bg(s)
    _header(s, "Impact measures", "How programme leads know debt is under control")
    _footer(s, 14)
    measures = [
        ...
        ("Gate maturity", "Warn-only → blocking → org-wide injection"),
        ("Nexus alignment", "Digest pins + approved version windows in force"),
    ]
    ...
    actions = [
        ...
        "Agree Nexus-aligned pin rules, then move CI from warn-only to blocking",
    ]
